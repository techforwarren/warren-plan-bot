...
for plan in plans_dict["plans"]:
    ...
    page_text = [t for t in page_soup.find_all(text=True) if t.parent.name not in blacklist]
    ...
    rake_results = r.run(parsed_text, minCharacters=MIN_CHARACTERS_IN_PHRASE, maxWords=MAX_WORDS_IN_PHRASE, minFrequency=MIN_WORD_FREQUENCY)
    key_phrases = []
...
print(json.dumps(keyword_dict, sort_keys=False, ensure_ascii=False, indent=4, separators=(',', ': ')))
